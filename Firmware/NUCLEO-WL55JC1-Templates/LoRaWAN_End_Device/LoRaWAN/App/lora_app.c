...
static void OnRxData(LmHandlerAppData_t *appData, LmHandlerRxParams_t *params)
{
  ...
  APP_LOG(TS_OFF, VLEVEL_H, "Something received\r\n");
	...
	if (LmHandlerIsBusy() == false)
	{
		...
		else if (LORAMAC_HANDLER_DUTYCYCLE_RESTRICTED == status)
		{
			...
			if (nextTxIn > 0)
			{
				...
				APP_LOG(TS_ON, VLEVEL_M, "\n\n----------- Device Reset due to Duty Cycle restrictions----------------\n\n");
				NVIC_SystemReset(); //Due to Duty Cycle time messages restrictions -> reset end device to "overcome" Lorawan rule
			}
		}
	}
...
static void OnJoinRequest(LmHandlerJoinParams_t *joinParams)
{
  ...
  if (joinParams != NULL)
  {
    if (joinParams->Status == LORAMAC_HANDLER_SUCCESS)
    {
      ...
      /*Change RX1 and RX2 delay when Joining*/
      LmHandlerSetRx1Delay(1000);
      LmHandlerSetRx2Delay(2000);

      //LmHandlerRequestClass(CLASS_B); //TEST changing class
      /*Change RX1 and RX2 delay when Joining*/
